...
def generate_profile_id(profile):
    "Hash the core profile fields so the same applicant always gets the same id."
    key_inputs = [
...
# ACT composite -> nearest SAT equivalent
...
def normalize_name(name, alias_map):
    "Map a name to its canonical form, or return it unchanged."
...
def normalize_list_column(series, alias_map):
    "Normalize every string in a column of lists."
...
EC_CATEGORY_MAP = {
    ...
    "baking": "Hobby/Club",
    "cooking": "Hobby/Club",
    "family responsibilit": "Family Responsibilities",  # partial-word match
}
...
AWARD_CATEGORY_MAP = {
    ...
    "olympiad": "Olympiad (USA)",  # generic fallback
...
def load_profiles(path="profiles.jsonl"):
    "Read the JSONL file into a cleaned DataFrame."
    rows = []
    try:
    ...
    # Some source rows pack several fields into one newline-joined string; flatten those.
    def clean_column(col_series, col_name):
        def extract_value(x):
            if isinstance(x, str) and '\n' in x:
                # e.g. "Politics\nRace:\nWhite\n..." — keep the first real line.
                return [x.split('\n')[0].strip()]
        ...
        # majors sometimes arrives as a bare string
    ...
    # Drop rows with no academic data at all (basically just an ID)
    df = df.dropna(subset=['gpa_unweighted', 'sat', 'act'], how='all')

    # Source IDs are unreliable and duplicated, so always rebuild them from content.
...
def augment_dataframe(df):
    "Add derived columns: counts, STEM flag, SAT equivalent, acceptance tiers."
    if df.empty:
        return df

    # Normalize names before deriving anything from them.
    ...
    def get_sat_equivalent(row):
        ...
        # A "SAT" of <= 36 is really an ACT score in the wrong field.
    ...
    # Acceptance tiers, per 2024-2025 US News national university rankings.
    t5 = [
